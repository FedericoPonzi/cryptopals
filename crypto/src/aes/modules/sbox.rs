...
pub fn single_sub(input: u8) -> u8 {
    SBOX[input as usize]
}

fn sub(state: &[u8; 16], map: &[u8; 256]) -> [u8; 16] {
    let mut ret = [0u8; 16];
    for (pos, el) in state.iter().cloned().map(usize::from).enumerate() {
        ret[pos] = map[el];
    }
    ret
}
...
mod test {
    ...
    fn test_sub_bytes() {
        ...
        assert_eq!(sub_bytes_inverse(&expected), state);
    }
}
